...
def generate_combined_ast_for_repo(repo_path):
    # Create a root node to combine all trees
    combined_tree = ast.Module(body=[])
    found_files = False

    # Walk through the repo and generate AST for each file
    for root, dirs, files in os.walk(repo_path):
        for file in files:
            if file.endswith(".py"):
                ...
                combined_tree.body.extend(tree.body)  # Append nodes to the combined tree

    if not found_files:
        print("No Python (.py) files found in the repository.")

    # Link parent nodes in the combined AST
    BatchAPIDetector().link_parent_nodes(combined_tree)
    return combined_tree  # Return the combined AST
...
class FunctionCallVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        ...
        detected_provider = detect_cloud_provider(trees)
...
def analyze_function_calls_in_repo(trees):
    ...
    all_misuses = set()  # Initialize a set to store all misuses across all files

    for file_path, tree in trees:
        visitor = FunctionCallVisitor(file_path)  # Pass file path to visitor
        visitor.visit(tree)  # Visit the AST
        total_misuse_count += visitor.call_count
        all_misuses.update(visitor.get_misuses())  # Add misuses to the collection

    # Print total occurrences summary
    print(f"Total occurrences of 'Not using Batch API for Data Processing' misuse found inside loops: {total_misuse_count}")
    return total_misuse_count, all_misuses  # Return total misuse count and all misuses
...
def detect_function_calls(trees, combined_tree):    
    ...
    misuses1, additional_misuse_count = detect(combined_tree)
